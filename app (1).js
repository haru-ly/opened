...
async function loadLyricsPage() {
  ...
  const myCompany = currentProfile?.company;
  ...
  // 임원진/대표는 모든 과제를 볼 수 있고, 연습생/직원은 본인 회사 + 본인이 대상인 과제만 본다
  ...
  snap.forEach(d => {
    ...
    if (!canSeeAll) {
      // 회사 불일치면 숨김
      if (a.company && a.company !== myCompany) return;
  ...
  unsubNotices = onSnapshot(
    ...
    (snap) => {
      const filtered = snap.docs.filter(d => {
        const n = d.data();
        return !n.company || n.company === myCompany;
      });
...
async function loadMembersTable() {
  ...
  snap.forEach(d => {
    ...
    // 대표가 아니면 본인 회사만, 대표는 현재 보고 있는 회사 기준 필터
